...
// groupId 받기
router.post('/index', async (req, res) => { // (사진 전체 응답) // calender, member 별 보관함에 사용
    let loginStatus = req.app.TokenUtils.verify(req.headers.token);
    if (!loginStatus)
    ...
    let res1 = await req.app.db.collection('EEHO').find({ groupId: new ObjectId(req.body.groupId) }).sort({ _id: -1 }).toArray();
...
// 사진 한장 받을 때 쓰는 거
router.post('/upload', upload.single("profile"), async (req, res) => { // (이미지, 받는 사람 ID, groupId)
    let loginStatus = req.app.TokenUtils.verify(req.headers.token);
    if (!loginStatus)
    ...
    receiver = receiver.filter(item => item !== loginStatus.id);
