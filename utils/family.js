...
router.post("/member/login", async (req, res) => {
  // (가족코드, 사용자이름, 푸시토큰) (code, userName, pushToken)
  if (!req.body.code)
    return res
      .status(400)
      .json({ ok: false, message: "you should put your code" });
  let result_family = await req.app.db
    .collection("family")
  ...
  if (result_family) {
    if (!req.body.userName)
      return res
        .status(400)
        .json({ ok: false, message: "you should put your userName" });
    ...
    let dataMine = result_family.user.find(
      (item) => item.userId.toString() === result_find._id.toString()
    );
    if (result_find) {
      const accessToken = req.app.TokenUtils.makeToken({
        id: String(result_find._id),
      });
      ...
      return res
        .status(500)
        .json({ ok: false, message: "non-existent userName!!!" });
    }
  } else {
    return res
      .status(500)
      .json({ ok: false, message: "non-existent familyName!!!" });
  }
});

router.post("/create", upload.single("profile"), async (req, res) => {
  // (가족이름, 사용자이름, 구성역할, 이미지, 푸시토큰) (familyName, userName, role, profile, pushToken)
  ...
  if (!result_user)
    return res
      .status(500)
      .json({ ok: false, message: "cannot insert user data" });
  ...
  if (!result_insert)
    return res
      .status(500)
      .json({ ok: false, message: "cannot insert family data" });
  try {
    await req.app.db
      .collection("user")
      .updateOne(
        { userName: req.body.userName, signDate: dateToday },
        { $set: { familyId: result_insert.insertedId } }
      );
    ...
    const accessToken = req.app.TokenUtils.makeToken({
      id: String(result_user.insertedId),
    });
    return res.status(200).json({
      ok: true,
    ...
    return res
      .status(500)
      .json({ ok: false, message: "internal sever error", error: error });
  }
});

router.post("/code/isExisted", async (req, res) => {
  // (코드) (code)
  if (!req.body.code)
    return res.status(400).json({ ok: false, message: "no_data" });
  let result_find = await req.app.db
    .collection("family")
    .findOne({ code: req.body.code });
  if (result_find) {
...
router.post("/participate", upload.single("profile"), async (req, res) => {
  // (코드, 사용자이름, 구성역할, 이미지, 푸시토큰) (code, userName, role, profile, pushToken)
  if (!(req.body.code && req.body.userName && req.body.role))
    return res
      .status(400)
      .json({ ok: false, message: "check your body again" });
  let result_find = await req.app.db
    .collection("family")
    .findOne({ code: req.body.code });
  ...
  if (result_find) {
    if (result_find.familyCount >= 5)
      return res.status(400).json({ ok: false, message: "max" });
    let checkName = result_find.user.find(
      (item) => item.userName.toString() === req.body.userName.toString()
    );
    if (checkName)
    ...
    try {
      ...
      return res
        ...
        .json({
          ok: false,
          message: "notification internal server error",
          error: error,
        });
    }
  } else {
